...
export async function recommendHospitals(input: RecommendHospitalsInput): Promise<RecommendHospitalsOutput> {
  ...
  console.log("Smart recommendations input (mock active):", input);
  return Promise.resolve({
    hospitals: ["City General Hospital (Mock Rec)", "Sunshine Medical Center (Mock Rec)", "Hope Children's Hospital (Mock Rec)"]
  });

  // Original flow call (commented out)
...
// The recommendHospitalsFlow is defined but not directly called when mock data is active.
// It can be reactivated by changing the recommendHospitals function above.
